...
#   marker-disease association
    ...
    for (i in 1:n.sim)
    {
       ...
       is.case <- (y==1)
       ca.j <- genecounting(g[is.case,],loci=loci,handle.miss=handle.miss)
       co.j <- genecounting(g[!is.case,],loci=loci,handle.miss=handle.miss)
  ...
  if (n.sim > 0)
  {
    if (!quietly)
    {
      cat("\nLRT = ",x2obs, " p = ",pobs, " sim p = ",psim/n.sim, "\n")
