...
We start with OpenBUGS and for illustrative purpose with the toy data in
section~\ref{toy} but impute the missing data for variable \texttt{qt}. Unfortunately,
there is mixed success of OpenBUGS across systems and here we but only show the R code
as follows,
\begin{verbatim}
library(gap)
set.seed(1234567)
