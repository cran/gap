...
To facilitate comparisons and individual preferences, I have made the source codes
available for 2LD, EHPLUS, GENECOUNTING, HAP which have enjoyed great popularity ahead
of the genomewide association studies (GWAS) therefore are likely to be more familiar
than their R couunterparts in \texttt{gap}. However, you need to follow their instructions
to compile for a particular computer system.\\
...
\includegraphics[scale=0.25,width=\linewidth]{figures/4w.pdf}
...
created for additional fatures such as centering the chromosome ticks, allowing for 
